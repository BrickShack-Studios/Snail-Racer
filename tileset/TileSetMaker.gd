...
onready var texture = $Sprite.texture # still runs without onready
	...
	for x in range(tex_width):
		for y in range(tex_height):
			var region =  Rect2(x * tile_size.x, y * tile_size.y,
								tile_size.x, tile_size.y)# unecessary multiplication
			var id = x + y * 10 
			ts.create_tile(id)
			ts.tile_set_texture(id, texture)
			ts.tile_set_region(id, region)
	ResourceSaver.save("res://tileset/CircuitTileSet.tres", ts)